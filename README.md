...
carrying `CriomeDaemonConfiguration` (socket + `criome.sema` store location),
with `Configured` / `ConfigurationRejected` / `RequestUnimplemented` replies.

Default builds carry `nota-text` for CLI/debug projection; the wire is
binary/rkyv. See `INTENT.md`, including the pending config-type reconciliation
with the criome daemon.
