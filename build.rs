...
const MODULE_PATH: &[&str] = &["meta_signal_criome", "lib"];
const SIGNAL_MODULE_PATH: &[&str] = &["signal_criome", "lib"];
...
impl SchemaBuild {
    ...
    fn run(&self) {
        ...
        .expect("assemble authority-approved owner Criome Interface transaction");
        let rust = rust_logos();
        let type_paths = CriomeRustTypePaths::new();

        BootstrapInterfaceGeneration::new(&assembly, &rust, &type_paths, &source_path, &rust_path)
            .generate()
            .expect("project owner Criome Interface from the verified transaction")
            .write_or_check("META_SIGNAL_CRIOME_UPDATE_INTERFACE_ARTIFACTS")
            .expect("checked-in owner Criome Interface source and Rust projection are fresh");
        CargoEthosSourceMetadata::new("meta-signal-criome")
...
fn imported_signal_seats() -> Vec<&'static signal_criome::bootstrap_manifest::DeclarationSeat> {
    let wanted = IMPORTED_SIGNAL_TYPES
        .iter()
        .copied()
        .collect::<BTreeSet<_>>();
    let seats = signal_criome::bootstrap_manifest::DECLARATION_SEATS
        .iter()
...
fn bootstrap_catalog() -> BootstrapCatalog {
    ...
    .expect("owner Criome bootstrap catalog is complete")
}
...
struct CriomeRustTypePaths(BTreeMap<VocabularyEncodedId, RustTypePath>);

impl CriomeRustTypePaths {
    fn new() -> Self {
        use bootstrap_manifest as manifest;
...
impl RustTypePathResolver for CriomeRustTypePaths {
